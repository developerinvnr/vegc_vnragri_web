...
if(isset($_POST['act']) && $_POST['act']=='getV_agr_report_list')
{
...
{	
 ...
 $hierarchyCond = "(agr.prod_person IN (".implode(', ', $allids).") or agr.prod_executive IN (".implode(', ', $allids)."))";	
	...
	for ($i=$agyearf; $i <= $agyeart; $i++) {
		...
		$qry.=" SELECT agr.agree_no, agree_date, ann_fscode_f, ann_fscode_m, u.uName, f.fname, f.father_name, o.oname, f.village_id, f.tahsil_id, f.distric_id, f.state_id, VisitDate, EstAcrYiels, StndAcr, EstYielsKg FROM agreement_remark rmk, `agreement_".$i."` agr, users u, farmers f, organiser o,village v, tahsil t, distric d, state s where f.village_id=v.VillageId and f.tahsil_id=t.TahsilId and f.distric_id=d.DictrictId and f.state_id=s.StateId and rmk.VisitDate between '".$from."' and '".$to."' and rmk.agree_no=agr.agree_no and agr.prod_person=u.uId and agr.second_party=f.fid and agr.org_id=o.oid and ".$cropCond." and ".$orgrCond." and ".$farmerCond." and ".$hierarchyCond." and ".$userCond." and ".$prodcode." and ".$pperson." and ".$keyArea." and ".$pexecutive." and ".$keyCond;
		if($i!=$agyeart){
			$qry.=' UNION';
		}
	...
	<tr>
		...
		<td style="text-align:left;"><?=ucwords(strtolower($agrd['oname']))?></td>
		<td style="text-align:left;"><?=ucwords(strtolower($agrd['fname']))?></td>
		<td style="text-align:left;"><?=ucwords(strtolower($agrd['father_name']))?></td>
		
		<td style="text-align:left;"><?=$agrd['VisitDate']?></td>
		<td style="text-align:left;"><?=$agrd['EstAcrYiels']?></td>
		<td style="text-align:left;"><?=$agrd['StndAcr']?></td>
		<td style="text-align:left;"><?=$agrd['EstYielsKg']?></td>
	</tr>
...
elseif(isset($_POST['act']) && $_POST['act']=='getV_report_list')
{
	...
	<tr>
		...
		<td style="text-align:left;"><?=ucwords(strtolower($agrd['oname']))?></td>
		<td style="text-align:left;"><?=ucwords(strtolower($agrd['fname']))?></td>
		<td style="text-align:left;"><input type="text" style="border:hidden;width:100%;" value="<?=ucwords(strtolower($agrd['father_name']))?>" /></td>
