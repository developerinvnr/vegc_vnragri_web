...
$allc=mysql_query("SELECT * FROM crop order by cropname");
...
$allu=mysql_query("SELECT * FROM `users` where uStatus='A' AND uStatus!='S' order by uName asc");
...
<div class="pagethings" style="width:85%;">
	...
	<div id="addcrtbl" style="margin-bottom: 10px;padding:2px;background-color: #EDEDED;border:2px solid #ccc;overflow:scroll;">
		...
		<h6 style="font-weight: bold;">Scheduled Visit Report</h6>
...
//elseif($rSe['Summer']=='Y'){ $Df=$Rdf; $Dt=$Rdt; }
...
<input type="hidden" id="Sdt" value="<?=$Sdt?>" />				
					...
					<th>&nbsp;Crop: &nbsp;&nbsp;&nbsp;<select id="crop" onchange="SelCrop(this.value)" style="width:110px;"><option value="">All</option>
						...
						<?php /*&nbsp;Prod<sup>n</sup> Code: <select id="prodcode" style="width:80px;"></select> */?>
						
						<input type="hidden" id="prodcode" style="width:80px;">
					...
					<script>
					 ...
					 { $.post("exportimportAjax.php",{ action:'get_prodcode',crop:crop},function(data){
...
<?php for($k=2022; $k<=$m; $k++){ $qrype.=" SELECT uId,uName from agreement_".$k." agr inner join users u on agr.prod_executive=u.uId group by u.uId"; if($k!=$m){$qrype.=' UNION ';} if($k==$m){$qrype.=' order by uName asc ';} } $run_qrype=mysql_query($qrype); while($respe=mysql_fetch_assoc($run_qrype)){echo '<option value='.$respe['uId'].'>'.ucfirst(strtolower($respe['uName'])).'</option>';}?></select>					
					...
					<script type="text/javascript">
					 ...
					 function exportagrixls(){
								...
								myWindow = window.open('SvisitedreportExp.php?from='+from+'&to='+to+'&crop='+crop+'&secondparty='+secondparty+'&orgr='+orgr+'&users='+users+'&keywordSearch='+keywordSearch+'&prodcode='+prodcode+'&pperson='+pperson+'&pexe='+pexe+'&sii='+sii+'&dii='+dii+'&tii='+tii+'&vii='+vii, '_blank', 'location=yes,height=300,width=300,scrollbars=yes,status=yes');
		...
		<table class=" estable table table-bordered" style="width:100%;">
			<thead>
				<tr>
					...
					<th rowspan="2" style="width:60px;">FSCode<br />Female / Male</th>
					...
					<th rowspan="2" style="width:100px;">Visit Date</th>
					<th rowspan="2" style="width:100px;">Estimated/ Acaeage Yield</th>
					<th rowspan="2" style="width:100px;">Standard Acreage</th>
					<th rowspan="2" style="width:100px;">Estimated Yield in kg</th>
				</tr> 
			</thead>
			<tbody id="reportBody">
...
<script>
	...
	function agreeSearch(page){
		...
		$.post("SvisitReportAjax.php",{ act:'getV_agr_report_list',page:page,crop:crop,orgr:orgr,from:from,to:to,secondparty:secondparty,users:users,keywordSearch:keywordSearch,prodcode:prodcode,pperson:pperson,pexe:pexe,sii:sii,dii:dii,tii:tii,vii:vii},function(data) {
	...
	$(document).ready(function(){
		...
		*/
		...
		if(from=='' && to==''){
			...
			$.post("SvisitReportAjax.php",{ act:'getV_report_list',from:week,to:d},function(data) {
